...
func main() {
	...
	//cfgPath = "D:\\transfer\\app-proc.yml"
	...
	if global.Cfg().InitStock {
		doStock()
	}

	if global.Cfg().InitPosition != "" && global.Cfg().InitBinlogFile != "" {
		_args = []string{global.Cfg().InitBinlogFile, global.Cfg().InitPosition}
		doPosition()
	}
