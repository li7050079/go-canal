...
type Rule struct {
	...
	DefaultColumnValueMap map[string]interface{}
	PaddingMap            map[string]*model.Padding
	AddColumnPaddingMap   map[string]interface{}
...
func RuleInsDel(ruleKey string) bool {
	_lockOfRuleInsMap.RLock()
	defer _lockOfRuleInsMap.RUnlock()

	delete(_ruleInsMap, ruleKey)
...
func (s *Rule) Initialize() error {
	...
	if err := s.buildAddColumnPaddingMap(); err != nil {
		return err
	}
...
func (s *Rule) buildAddColumnPaddingMap() error {

	addPaddingMap := make(map[string]interface{})
	if s.AddColumn != "" {
		ls := strings.Split(s.AddColumn, ",")
		for _, t := range ls {
			paddingMap := make(map[string]*model.Padding)
			cmc := strings.Split(t, "=")
			if len(cmc) != 2 {
				return errors.Errorf("add_column format error in rule")
			}
			column := cmc[0]
			cache := cmc[1]
			if column != "" && cache != "" {
				test := strings.Split(cache, ".")
				localValue := strings.ReplaceAll(strings.ReplaceAll(test[1], "{", ""), "}", "")
				localValues := strings.Split(localValue, ":")
				cacheKey := strings.ReplaceAll(strings.ReplaceAll(test[0], "{", ""), "}", "")
				localColumn, index := s.TableColumn(localValues[0])
				if index < 0 {
					errors.Errorf("column key can't null ")
					continue
				}
				wrapName := ""
				if len(localValues) > 1 {
					wrapName = localValues[1]
				}
				...
				padding := &model.Padding{
					...
					ColumnType:     localColumn.Type,
					ColumnMetadata: localColumn,
				}
				paddingMap[cacheKey] = padding
				addPaddingMap[column] = paddingMap
			} else {
				return errors.Errorf("add_column key value can't null ")
			}
		}
		s.AddColumnPaddingMap = addPaddingMap
	}
	return nil
